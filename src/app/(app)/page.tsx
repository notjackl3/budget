...
import { monthlySummary, cashflowByMonthForYear } from "@/lib/aggregate";
...
export default async function DashboardPage({
  ...
  const cashflow = cashflowByMonthForYear(agg, year);
  ...
  // Income for the month. Prefer money actually recorded as income this month;
  // if none was recorded but jobs are configured, fall back to expected pay so
  // the net/savings analysis still works. `usingExpected` flags which one.
  const expectedMonthlyCents = incomeTotals(jobs).monthlyCents;
  const usingExpected = summary.incomeCents === 0 && expectedMonthlyCents > 0;
  const incomeCents = usingExpected ? expectedMonthlyCents : summary.incomeCents;
    ...
    <div className="rise-stagger mx-auto max-w-5xl space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard
          ...
          sublabel={usingExpected ? "expected (from jobs)" : "received this month"}
        />
        <StatCard
          label="Spent"
      ...
      {/* Income detail: how the money-in breaks down, vs. expected pay. */}
      {(summary.incomeCents > 0 || expectedMonthlyCents > 0) && (
        <Card>
          ...
          <CardContent>
            {summary.incomeByType.length > 0 ? (
              <div className="flex flex-wrap gap-x-6 gap-y-1 text-sm text-muted-foreground">
                {summary.incomeByType.map((t) => (
                  <span key={t.type}>
                    {t.type}{" "}
              ...
              <p className="text-sm text-muted-foreground">
                No income transactions recorded for {monthLabel(month)}.
              </p>
            )}
            {expectedMonthlyCents > 0 && (
              <p className="mt-3 text-xs text-muted-foreground">
                Expected from jobs:{" "}
                <span className="tabular font-medium text-foreground">
                  {money(expectedMonthlyCents)}
                </span>{" "}
                / mo.{" "}
                {!usingExpected &&
                  summary.incomeCents > 0 &&
                  (summary.incomeCents >= expectedMonthlyCents
                    ? `Received ${money(summary.incomeCents - expectedMonthlyCents)} above plan.`
                    : `${money(expectedMonthlyCents - summary.incomeCents)} short of plan.`)}
              </p>
            )}
          </CardContent>
        </Card>
